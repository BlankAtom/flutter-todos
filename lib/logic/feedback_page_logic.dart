...
class FeedbackPageLogic {
  ...
  void onFeedbackSubmit(FeedbackWallPageModel feedbackWallPageModel) async {
    ...
    if (!canSubmitSuggest) {
      showWrongDialog(
          context, IntlLocalizations.of(context).feedbackFrequently);
      return;
    }

    final userName =
        await SharedUtil.instance.getString(Keys.currentUserName) ??
            IntlLocalizations.of(context).noName;
...
//    String transFormName = Uri.encodeFull(fileName).replaceAll("%", "");
    ...
    final connectWay =
        "${(_model.contactWay ?? "") + "<emoji>${_model.currentSelectSvg + 1}<emoji>"}";
    final account =
            ...
            ApiService.instance.postSuggestionWithAvatar(
              ...
              success: (bean) {
                SharedUtil.instance.saveString(
                    Keys.lastSuggestTime, DateTime.now().toIso8601String());
  ...
  Future<bool> canSubmit() async {
    String lastTime =
        await SharedUtil.instance.getString(Keys.lastSuggestTime) ?? "";
    if (lastTime.isEmpty) {
      return true;
    } else {
      ...
      if (diff.inHours.abs() < 8) {
        ...
        SharedUtil.instance
            .saveString(Keys.lastSuggestTime, now.toIso8601String());
        return true;
      }
    }
  }
