...
class SynchronizeWidget extends StatefulWidget {
  ...
  const SynchronizeWidget({required Key key, required this.mainPageModel})
      : super(key: key);
...
class _SynchronizeWidgetState extends State<SynchronizeWidget> {
          ...
          child: needSyncedLength == 0
              ...
              : Column(
                  ...
                  children: <Widget>[
                    ...
                    Text(
                      IntlLocalizations.of(context).clickToSyn,
        ...
        return Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(
                IntlLocalizations.of(context).synchronizing,
                style: TextStyle(color: Colors.white),
              ),
              Text(
                "(${needSynTasks.length} / $needSyncedLength)",
                style: TextStyle(color: Colors.white, fontSize: 12),
              ),
            ],
          ),
        ...
        return Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(
                IntlLocalizations.of(context).cloudSynchronizing,
                style: TextStyle(color: Colors.white, fontSize: 12),
              ),
            ],
          ),
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: <Widget>[
                ...
                Text(
                  IntlLocalizations.of(context).synchronizeFailed,
                  style: TextStyle(color: Colors.white),
                )
              ],
            ),
  ...
  void postUpdateTask(TaskBean taskBean, String token) async {
    ...
    ApiService.instance.postUpdateTask(
      success: (CommonBean bean) {
        taskBean.uniqueId != null ? syncedList.add(taskBean.uniqueId!) : null;
        taskBean.needUpdateToCloud = 'false';
        updateLocalTasks(token!);
      },
  ...
  void clickToSyn() async {
    if (synFlag == SynFlag.synchronizing ||
        synFlag == SynFlag.cloudSynchronizing) return;
    setState(() {
    ...
    ApiService.instance.login(
      ...
      success: (LoginBean loginBean) {
        ...
        SharedUtil.instance.saveString(Keys.account, account).then((value) {
          SharedUtil.instance
              .saveString(Keys.currentUserName, loginBean.username);
          ...
          if (loginBean.avatarUrl != null) {
            ...
            String localAvatarFileName =
                widget.mainPageModel.currentAvatarUrl.split("/").last;
            if (cloudAvatarFileName != localAvatarFileName) {
              SharedUtil.instance.saveString(Keys.netAvatarPath,
                  ApiStrategy.baseUrl + loginBean.avatarUrl);
              SharedUtil.instance
                  .saveInt(Keys.currentAvatarType, CurrentAvatarType.net);
              widget.mainPageModel.currentAvatarUrl =
                  ApiStrategy.baseUrl + loginBean.avatarUrl;
